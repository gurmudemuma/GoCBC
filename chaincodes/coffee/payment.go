...
func (c *CoffeeContract) InitiatePayment(ctx contractapi.TransactionContextInterface,
	...
	if err != nil {
		return fmt.Errorf("InitiatePayment: invalid amount: %w", err)
	}
	...
	if err := ValidateNonEmptyString(receivingBank, "receivingBank", MaxStringLen); err != nil {
		return fmt.Errorf("InitiatePayment: %w", err)
	}
	if err := ValidateNonEmptyString(beneficiaryAccount, "beneficiaryAccount", MaxIDLen); err != nil {
		return fmt.Errorf("InitiatePayment: %w", err)
	}
	...
	if existingPayment != nil {
		return fmt.Errorf("InitiatePayment: payment %s already exists", paymentID)
	}
...
func (c *CoffeeContract) SubmitPaymentDocuments(ctx contractapi.TransactionContextInterface,
	...
	// VALIDATION: ID
	if err := ValidateID(paymentID, "paymentID"); err != nil {
		return fmt.Errorf("SubmitPaymentDocuments: %w", err)
	}

	paymentJSON, err := ctx.GetStub().GetState("PAYMENT_" + paymentID)
	if err != nil {
		return fmt.Errorf("SubmitPaymentDocuments: failed to read payment %s: %w", paymentID, err)
	}
	if paymentJSON == nil {
		return fmt.Errorf("SubmitPaymentDocuments: payment %s does not exist", paymentID)
	}
	...
	if payment.Status != "PENDING" {
		return fmt.Errorf("SubmitPaymentDocuments: documents cannot be submitted for payment %s, current status: %s", paymentID, payment.Status)
	}
...
func (c *CoffeeContract) VerifyPaymentDocuments(ctx contractapi.TransactionContextInterface,
	...
	if err := ValidateNonEmptyString(verifiedBy, "verifiedBy", MaxStringLen); err != nil {
		return fmt.Errorf("VerifyPaymentDocuments: %w", err)
	}

	paymentJSON, err := ctx.GetStub().GetState("PAYMENT_" + paymentID)
	if err != nil {
		return fmt.Errorf("VerifyPaymentDocuments: failed to read payment %s: %w", paymentID, err)
	}
	if paymentJSON == nil {
		return fmt.Errorf("VerifyPaymentDocuments: payment %s does not exist", paymentID)
	}
	...
	if payment.Status != "DOCUMENTS_SUBMITTED" {
		return fmt.Errorf("VerifyPaymentDocuments: documents cannot be verified for payment %s, current status: %s", paymentID, payment.Status)
	}
...
func (c *CoffeeContract) SettlePayment(ctx contractapi.TransactionContextInterface,
	...
	paymentJSON, err := ctx.GetStub().GetState("PAYMENT_" + paymentID)
	if err != nil {
		return fmt.Errorf("SettlePayment: failed to read payment %s: %w", paymentID, err)
	}
	if paymentJSON == nil {
		return fmt.Errorf("SettlePayment: payment %s does not exist", paymentID)
	}
	...
	// VALIDATION: Retention rate percentage
...
// QueryAllPayments - Get all payment settlements in the system
func (c *CoffeeContract) QueryAllPayments(ctx contractapi.TransactionContextInterface) ([]*PaymentSettlement, error) {
	
	queryString := `{"selector":{"paymentId":{"$exists":true}}}`

	resultsIterator, err := ctx.GetStub().GetQueryResult(queryString)
	if err != nil {
		return nil, fmt.Errorf("failed to query all payments: %v", err)
	}
	defer resultsIterator.Close()
	...
	for resultsIterator.HasNext() {
		queryResponse, err := resultsIterator.Next()
		if err != nil {
			return nil, fmt.Errorf("failed to iterate payments: %v", err)
		}

		var payment PaymentSettlement
		err = json.Unmarshal(queryResponse.Value, &payment)
		if err != nil {
			log.Printf("Warning: failed to unmarshal payment %s: %v", queryResponse.Key, err)
...
// Added July 2, 2026 - CBE Section 3.2.iv implementation
// LIMITED TO: Fruits, Flowers, Meat only

type Consignment struct {
	ConsignmentID     string    `json:"consignmentId"`     // Unique consignment ID
	ExporterID        string    `json:"exporterId"`        // Exporter/Consignor
	CommodityType     string    `json:"commodityType"`     // FRUITS, FLOWERS, MEAT (only these allowed)
	Description       string    `json:"description"`       // Commodity description
	BuyerName         string    `json:"buyerName"`         // Consignee/Agent name
	BuyerAddress      string    `json:"buyerAddress"`      // Consignee address
	BuyerCountry      string    `json:"buyerCountry"`      // Destination country
	PermitAmount      float64   `json:"permitAmount"`      // Authorized permit value (USD)
	Currency          string    `json:"currency"`          // Default: USD
	SettledAmount     float64   `json:"settledAmount"`     // Amount actually received after sale
	OutstandingAmount float64   `json:"outstandingAmount"` // Difference (permitAmount - settledAmount)
	Status            string    `json:"status"`            // PERMITTED, SHIPPED, SOLD, SETTLED, OUTSTANDING
	PermitNumber      string    `json:"permitNumber"`      // NBE permit reference
	PermitDate        time.Time `json:"permitDate"`        // When permit issued
	ShipmentDate      string    `json:"shipmentDate"`      // When goods shipped
	SalesDate         string    `json:"salesDate"`         // When goods sold abroad
	SettlementDate    string    `json:"settlementDate"`    // When payment received
	SettlementDueDate string    `json:"settlementDueDate"` // Deadline for settlement (typically 90 days)
	SalesReports      []string  `json:"salesReports"`      // Sales reports from agent
	ReceivingBank     string    `json:"receivingBank"`     // Ethiopian bank
	ReceivingBankBIC  string    `json:"receivingBankBic"`  // SWIFT BIC
	PaymentReferences []string  `json:"paymentReferences"` // Multiple SWIFT references (partial payments)
	Warnings          []string  `json:"warnings"`          // Outstanding balance warnings
	CreatedAt         time.Time `json:"createdAt"`
	UpdatedAt         time.Time `json:"updatedAt"`
}

// ==================== DOCUMENTARY COLLECTION (CAD) FUNCTIONS ====================

// RegisterDocumentaryCollection - Register CAD transaction (URC 522 compliant)
func (c *CoffeeContract) RegisterDocumentaryCollection(ctx contractapi.TransactionContextInterface,
	collectionID, contractID, exporterID, drawerName, draweeName, draweeAddress,
	amountStr, currency, paymentTerm, acceptanceDaysStr, remittingBank, remittingBankBIC,
	collectingBank, collectingBankBIC, instructions string, documents []string) error {
	...
	acceptanceDays, _ := strconv.Atoi(acceptanceDaysStr)
	if paymentTerm == "ACCEPTANCE" && acceptanceDays <= 0 {
		return fmt.Errorf("acceptance days required for ACCEPTANCE term")
	}

	// Check if collection already exists
	existingCAD, err := ctx.GetStub().GetState("CAD_" + collectionID)
	if err != nil {
		return fmt.Errorf("failed to read collection: %v", err)
	}
	if existingCAD != nil {
		return fmt.Errorf("documentary collection %s already exists", collectionID)
	}
	...
	err = ctx.GetStub().PutState("CAD_"+collectionID, collectionJSON)
	if err != nil {
		return err
	}
	...
	// Create audit log
	changes := []FieldChange{
		{FieldName: "collectionId", OldValue: "", NewValue: collectionID, DataType: "string"},
		{FieldName: "amount", OldValue: "", NewValue: amountStr, DataType: "number"},
		{FieldName: "paymentTerm", OldValue: "", NewValue: paymentTerm, DataType: "string"},
		{FieldName: "status", OldValue: "", NewValue: "SENT", DataType: "string"},
	}
	...
	compliance := ComplianceMetadata{
		...
		EUDRCompliance: true,
		ICOCompliance:  true,
		ComplianceNote: fmt.Sprintf("Documentary collection registered per URC 522, %s payment", paymentTerm),
	}

	err = c.CreateAuditLog(ctx, "CREATE", "CAD", collectionID, "", "SENT", changes,
		fmt.Sprintf("Documentary collection sent to %s", collectingBank), compliance)
	if err != nil {
		log.Printf("WARNING: Failed to create audit log: %v", err)
	}

	fmt.Printf("✅ Documentary Collection registered: %s (%s, %s %.2f)\n",
		collectionID, paymentTerm, currency, amount)
	return nil
}

// UpdateCADStatus - Update documentary collection status
func (c *CoffeeContract) UpdateCADStatus(ctx contractapi.TransactionContextInterface,
	collectionID, newStatus, notes string) error {

	validStatuses := map[string]bool{
		"SENT": true, "FORWARDED": true, "PRESENTED": true,
		"ACCEPTED": true, "PAID": true, "REJECTED": true,
	}
	if !validStatuses[newStatus] {
		return fmt.Errorf("invalid CAD status: %s", newStatus)
	}

	collectionJSON, err := ctx.GetStub().GetState("CAD_" + collectionID)
	if err != nil {
		return fmt.Errorf("failed to read collection: %v", err)
	}
	...
	fmt.Printf("CAD %s status updated: %s → %s\n", collectionID, oldStatus, newStatus)
	return nil
}

// RecordCADPayment - Record payment received for documentary collection
func (c *CoffeeContract) RecordCADPayment(ctx contractapi.TransactionContextInterface,
	collectionID, paymentReference, amountReceivedStr string) error {
	...
	collection.PaymentDate = txTime.Format(time.RFC3339)
	collection.Remarks = "Payment received: " + paymentReference
	collection.UpdatedAt = txTime
	...
	fmt.Printf("✅ CAD payment recorded: %s (%.2f %s, Ref: %s)\n",
		collectionID, amountReceived, collection.Currency, paymentReference)
	return nil
}

// QueryDocumentaryCollectionsByExporter - Get all CAD collections for exporter
func (c *CoffeeContract) QueryDocumentaryCollectionsByExporter(ctx contractapi.TransactionContextInterface,
